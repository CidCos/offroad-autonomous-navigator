...
class MujocoOffroadEnvVision(gym.Env[dict[str, np.ndarray], np.ndarray]):
    """MuJoCo environment combining depth-camera perception with vehicle/goal state."""
    ...
    def step(
        ...
        distance_to_goal = euclidean_distance(
            new_state.x, new_state.y, self.config.goal_x, self.config.goal_y
        )

        return (
            self._build_observation(new_state),
            reward,
            terminated,
            truncated,
            {
    ...
    def _is_out_of_bounds(self) -> bool:
        ...
        return (
            self._state.x < self.config.map_min_x
            or self._state.x > self.config.map_max_x
            or self._state.y < self.config.map_min_y
            or self._state.y > self.config.map_max_y
        )
    ...
    def _is_goal_reached(self) -> bool:
        ...
        distance = euclidean_distance(
            self._state.x, self._state.y, self.config.goal_x, self.config.goal_y
        )
