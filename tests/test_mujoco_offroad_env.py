...
from offroad_autonomous_navigator.envs.mujoco.mujoco_offroad_env import MujocoOffroadEnv
from offroad_autonomous_navigator.envs.schemas import EnvConfig, RewardConfig


# 1
def test_reset_returns_valid_observation(default_env: MujocoOffroadEnv) -> None:
...
# 2
def test_step_before_reset_raises_runtime_error(default_env: MujocoOffroadEnv) -> None:
...
def test_truncation_on_max_episode_steps(default_config: EnvConfig, 
    ...
    env = MujocoOffroadEnv(config=config, reward_config=default_reward_config)
    env.reset()
    zero_action = np.array([0.0, 0.0], dtype=np.float32)
...
# 6
def test_normal_step_continues_episode(default_env: MujocoOffroadEnv) -> None:
...
# 7
def test_step_returns_correct_info(default_env: MujocoOffroadEnv) -> None:
